...
# Taken and adapted from https://github.com/20tab/bvh-python/blob/master/bvh.py
...
class Bvh:
    ...
    def get_joints_names(self):
        '''
        Get the name of all joints in the model (like "Head", "Neck", etc.)
        '''
        joints = []
    ...
    def joint_channels(self, name):
        '''
        Get the name of all channels for a given joint name (like "Xposition", "Yposition", etc.)
        '''
        joint = self.get_joint(name)
    ...
    def frame_joint_channel(self, frame_index, joint, channel, value=None):
        '''
        Get the value of a channel of a joint at any given frame
        '''
    ...
    def nframes(self):
        '''
        Get the toal number of frames in the bvh file
        '''
        try:
...
if __name__ == "__main__":
    bvh_file = open("walk-01-normal-azumi.bvh", "r")
    bvh_contents = bvh_file.read()
    bvh_file.close()
    bvh = Bvh(bvh_contents)
    # print(bvh.get_joints_names()) # Get the names of all joints in the model
    # print(bvh.joint_channels("Head")) # Get the channels of a given joint
    print(bvh.frame_joint_channel(0, "Head", "Xposition")) # Get the value of a channel (Xposition) of a joint (Head) at frame 0
