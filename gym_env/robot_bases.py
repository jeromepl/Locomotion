...
# TAKEN FROM https://github.com/bulletphysics/bullet3/blob/master/examples/pybullet/gym/pybullet_envs/robot_bases.py
	...
	def addToScene(self, bullet_client, bodies):
		...
		for i in range(len(bodies)):
			...
			for j in range(self._p.getNumJoints(bodies[i])):
				...
				part_name=jointInfo[12] # link name
				...
				if joint_name[:6] == "ignore":
					Joint(self._p, joint_name, part_name, bodies, i, j).disable_motor()
					continue

				joints[joint_name] = Joint(self._p, joint_name, part_name, bodies, i, j)
	...
	def reset(self, bullet_client):
		...
		if (self.doneLoading==0):
			...
			if self.self_collision:
				...
				self.objects = self._p.loadMJCF(os.path.join("res", self.model_xml), flags=pybullet.URDF_USE_SELF_COLLISION|pybullet.URDF_USE_SELF_COLLISION_EXCLUDE_ALL_PARENTS)
...
class Joint:
	def __init__(self, bullet_client, joint_name, link_name, bodies, bodyIndex, jointIndex):
		self.bodies = bodies
		self._p = bullet_client
		self.bodyIndex = bodyIndex
